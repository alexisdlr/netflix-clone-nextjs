
interface InputProps {
  id: string,
  onChange: any,
  type?: string, 
  value: string,
  label: string

}
const Input = ({id, onChange, type, value, label}: InputProps) => {
  return (
    <div className="relative">
      <input
        id={id}
        type={type}
        onChange={onChange}
        value={value}
        placeholder=" "
        className="block w-full text-white rounded-md px-6 pt-6 pb-1 text-md bg-neutral-700 appearence-none focus:outline-none focus:ring-0 peer"
      />
      <label htmlFor={id} className="absolute text-md text-zinc-400 duration-150 transform -translate-y-3 scale-75 top-4 z-10 origin-[0] left-6 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-3">
        {label}
      </label>
    </div>
  );
};

export default Input;
